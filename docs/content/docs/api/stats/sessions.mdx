...
<div className="flex flex-col gap-2 my-6">
  ...
  <a href="#get-session" className="flex items-center gap-3 p-3 rounded-lg bg-fd-secondary/50 hover:bg-fd-secondary transition-colors no-underline">
    <span className="px-2 py-1 text-xs font-bold rounded bg-green-500/20 text-green-600 dark:text-green-400">GET</span>
    <code className="text-sm">/api/session/:sessionId/:site</code>
  </a>
...
```
GET /api/session/:sessionId/:site
```
...
<Tabs items={['cURL', 'JavaScript', 'Python']}>
  <Tab value="cURL">
```bash title="Request"
curl -X GET "https://api.rybbit.io/api/session/sess_abc123/123?limit=50" \
  -H "Authorization: Bearer your_api_key_here"
```
  </Tab>
  <Tab value="JavaScript">
```javascript title="Request"
const response = await fetch(
  'https://api.rybbit.io/api/session/sess_abc123/123?limit=50',
  {
    headers: {
      'Authorization': 'Bearer your_api_key_here'
    }
...
response = requests.get(
    'https://api.rybbit.io/api/session/sess_abc123/123',
    params={'limit': 50},
    headers={
        'Authorization': 'Bearer your_api_key_here'
    }
